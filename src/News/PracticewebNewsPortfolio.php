...
class PracticewebNewsPortfolio extends ET_Builder_Module {
    function init() {
        $this->name       = esc_html__( 'Filterable Practiceweb News', 'practiceweb' );
        $this->slug       = 'et_pb_pw_news_portfolio';
    ...
    function get_fields() {
        $fields = array(
            ...
            '__project_terms' => array(
                'type'                => 'computed',
                'computed_callback'   => array( __CLASS__, 'get_portfolio_terms' ),
            ...
            '__projects' => array(
                'type'                => 'computed',
                'computed_callback'   => array( __CLASS__, 'get_portfolio_item' ),
                'computed_depends_on' => array(
                    'show_pagination',
                    'posts_number',
                    'include_categories',
    ...
    static function get_portfolio_item( $args = array(), $conditional_tags = array(), $current_page = array() ) {
        ...
        if( $query->have_posts() ) {
            ...
            while ( $query->have_posts() ) {
                ...
                $categories_object = get_the_terms( get_the_ID(), 'PracticeWEBContent' );
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        if( $projects->post_count > 0 ) {
            while ( $projects->have_posts() ) {
                ...
                $categories = get_the_terms( get_the_ID(), 'PracticeWEBContent' );
                if ( $categories ) {
                ...
                <div id="post-<?php the_ID(); ?>" <?php post_class( $main_post_class ); ?>>
                    ...
                    $post_meta = get_the_term_list( get_the_ID(), 'PracticeWEBContent', '', ', ' );
                    $thumbnail = get_thumbnail( $width, $height, $classtext, $titletext, $titletext, false, 'Blogimage' );
        ...
        $terms = get_terms( 'PracticeWEBContent', $terms_args );
